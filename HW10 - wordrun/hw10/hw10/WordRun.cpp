...
void WordRun::setWords(){
    std::ifstream wrdoth("/Users/lanceaquino/Desktop/PIC10A/HW10 - wordrun/hw10/hw10/word4.txt"); // change to just "word4.txt" when testing on vs code
...
void WordRun::guess(const std::string & user_guess){
    ...
    if (copy_of_user_guess.size() != chosen.size()){ //checks to see if the word is wrong size (input.size() != chosen.size())
        ...
        for (size_t guess_char_index = 0 ; guess_char_index <= final_char_index ; ++guess_char_index){ // main loop iterates through each character of the guess
...
std::string WordRun::randword(){ // remove before final build
    return chosen;
}
...
void WordRun::setScore(){
    ...
    int guess_limit = 150;
    int time_taken = getTime(); // gets the time that the user took in order to find the correct answer.
    int TimeScore = std::max(0,time_limit - time_taken); // calculates the time score. If the time goes over time_limit seconds, then the TimeScore is 0
    int GuessScore = std::max(0, guess_limit-guesses);  // calculates the guess score. If the number of guesses goes over guess_limit, then the GuessScore is 0
