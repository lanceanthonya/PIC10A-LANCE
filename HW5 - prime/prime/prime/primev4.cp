...
int main() {
    ...
    for (unsigned long long int current_num = low_bound; current_num<=up_bound; ++current_num){ // iterator that starts with the lower bound and ends at the upper bound. Each iteration increases the current_num, or the current number being prime factored, by 1
        ...
        for (int divisor = 2; final_num>=1;){ // starting with a divisor of 2, this loop continues until final_num<=1
            if(final_num%divisor == 0 && current_num != 2 && final_num != divisor){ // If the divisor divides evenly into final_num, prints the divisor. Does not execute if current_num is 2, which is a prime number but yields a remainder of 0 under this condition
                final_num /= divisor;
                cout << divisor << " x ";
            }
            else if (final_num == divisor && current_num!=2) { // when final_num becomes the same as the divisor, it means that this is the final divisor in the sequence, so the current for loop breaks and the next number in the defined range can be analyzed
                cout << divisor << "\n";
                break;
            }
    ...
    cout<< "\nElapsed Time: "<< duration.count()/1000. <<" ms\n";
    return 0;
}
