...
// This version is prime.cpp, except it uses a while loop on line 40
...
int main() {
    ...
    for (unsigned long long int current_num = low_bound; current_num<=up_bound; ++current_num){ // iterator that starts with the lower bound and ends at the upper bound. Each iteration increases the current_num, or the current number being prime factored, by 1
        ...
        for (int divisor = 2; final_num>=1;){ // starting with a divisor of 2, this loop continues until final_num<=1
            ...
            else if (divisor>final_num/2 && final_num == current_num){ // if the number stored in final_num never changes and the divisor is greater than the final_num divided by 2, then current_num was a prime
                cout <<"prime!\n";
                break;
            }
            ...
            else { // increases the divisor if it currently does not go into the current iteration of final_num
                ++divisor;
            }
